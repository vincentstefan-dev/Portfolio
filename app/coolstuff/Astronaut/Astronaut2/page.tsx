...
const dialogueLine =
  "Oh but who am I without all that I have been through...";

/* Main wallpaper stays fixed */
const mainBackground = {
  src: "/Images/Astronaut/crater.png",
  alt: "Pixel astronaut crater scene",
};

/* Small center-screen carousel */
const screenImages = [
  {
    src: "/Images/Astronaut/family.png",
    alt: "Pixel astronaut portrait",
  },
  {
    src: "/Images/Astronaut/mendelbrot.png",
    alt: "Pixel astronaut intro",
  },
  {
    src: "/Images/Astronaut/florifundia.png",
    alt: "Pixel crater scene",
  },
    {
    src: "/Images/Astronaut/eye1.png",
    alt: "Pixel crater scene",
  },
      {
    src: "/Images/Astronaut/sil1.png",
    alt: "Pixel crater scene",
  },
        {
    src: "/Images/Astronaut/cry.png",
    alt: "Pixel crater scene",
  },
          {
    src: "/Images/Astronaut/family2.png",
    alt: "Pixel crater scene",
  },
];
...
export default function AstronautIntroSection() {
  ...
  const activeScreenImage = screenImages[activeScreenIndex];
  ...
  useEffect(() => {
    const screenInterval = window.setInterval(() => {
      setActiveScreenIndex((currentIndex) => {
        return (currentIndex + 1) % screenImages.length;
      });
    }, 1800);

    return () => window.clearInterval(screenInterval);
  }, []);
  ...
  return (
    <section className="relative min-h-screen w-full overflow-hidden bg-black text-[#f1dcc0]">
      ...
      {/* Center nested carousel screen */}
      <div className="absolute left-1/2 top-[38%] z-20 w-[min(58vw,520px)] -translate-x-1/2 -translate-y-1/2">
        <div className="mini-screen-frame relative overflow-hidden rounded-[18px] border border-[#f8e0bd]/35 p-3">
          {/* screen glow */}
          <div className="pointer-events-none absolute inset-0 z-20 bg-[radial-gradient(circle_at_center,rgba(255,224,189,0.16),transparent_62%)]" />

          {/* glass shine */}
          <div className="pointer-events-none absolute left-4 right-4 top-4 z-30 h-[28%] rounded-[14px] bg-gradient-to-b from-white/24 to-transparent opacity-70" />

          {/* inner screen */}
          <div className="relative aspect-[16/9] overflow-hidden rounded-[12px] border border-white/12 bg-black">
            <img
              key={activeScreenImage.src}
              src={activeScreenImage.src}
              alt={activeScreenImage.alt}
              className="h-full w-full object-cover object-center image-render-pixel mini-screen-image"
              draggable={false}
            />

            {/* mini CRT scanlines */}
            <div className="pointer-events-none absolute inset-0 opacity-[0.22] mini-scanlines" />
            ...
            <div className="absolute left-4 top-4 z-30">
              <p className="mini-screen-label text-[0.55rem] tracking-[0.16em] text-[#f8e0bd]/80">
                MEMORY...{String(activeScreenIndex + 1).padStart(2, "0")}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="absolute bottom-[7%] left-1/2 z-20 flex w-[min(94vw,1160px)] -translate-x-1/2 items-stretch gap-4">
        ...
        <nav className="command-pane relative flex min-h-[150px] w-[min(34vw,330px)] flex-col justify-center gap-7 overflow-hidden rounded-[22px] border border-[#f8e0bd]/35 px-8 py-6 shadow-[0_0_35px_rgba(0,0,0,0.65)]">
          ...
          <Link
            href="/coolstuff/Astronaut/Astronaut3"
      ...
      <style jsx>{`
        ...
        .mini-screen-image {
          animation: mini-image-fade 500ms steps(4, end);
        }
        ...
        .mini-screen-label {
          text-shadow:
            2px 2px 0 #1d1712,
            0 0 10px rgba(255, 224, 189, 0.65);
        }

        @keyframes mini-image-fade {
          from {
            opacity: 0.25;
